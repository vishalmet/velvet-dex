...
import { logSwapInitiated, logSwapSuccess, logSwapError, logWalletConnected } from './firebase';
...
export const swapTokenMetaSolver = async (
  ...
  try {
    ...
    logSwapInitiated(sellTokenAddress, buyTokenAddress, amount);
    ...
    logWalletConnected(userAddress, chainId);
    ...
    logSwapSuccess(sellTokenAddress, buyTokenAddress, amount, receipt.transactionHash);
    return receipt;
  } catch (error) {
    logSwapError(sellTokenAddress, buyTokenAddress, amount, error);
    throw error;
  }
};
